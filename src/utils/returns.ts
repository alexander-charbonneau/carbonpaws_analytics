...
interface ReturnMetrics {
  ...
  CarbonPawsReturn: number // netReturn - hodlReturn
  impLoss: number
  fees: number
}
...
function formatPricesForEarlyTimestamps(position): Position {
  if (position.timestamp < PRICE_DISCOVERY_START_TIMESTAMP) {
    ...
    // WETH price
    if (position.pair?.token0.id === '0x6b3bd0478df0ec4984b168db0e12a539cc0c83cd') {
      position.token0PriceUSD = 1
    }
    if (position.pair?.token1.id === '0x6b3bd0478df0ec4984b168db0e12a539cc0c83cd') {
      position.token1PriceUSD = 1
    }
  }
  return position
...
export function getMetricsForPositionWindow(positionT0: Position, positionT1: Position): ReturnMetrics {
  ...
  const CarbonPaws_return = difference_fees_usd + imp_loss_usd
  ...
  return {
    ...
    CarbonPawsReturn: CarbonPaws_return,
...
export async function getLPReturnsOnPair(user: string, pair, ethPrice: number, snapshots) {
  ...
  for (const index in snapshots) {
    ...
    CarbonPawsReturn = CarbonPawsReturn + results.CarbonPawsReturn
