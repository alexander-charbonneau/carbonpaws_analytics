...
export function getPoolLink(token0Address, token1Address = null, remove = false) {
  if (!token1Address) {
    return (
      `https://carbonpaws.io/exchange/` +
      (remove ? `remove` : `add`) +
      `/${token0Address === '0x6b3bd0478df0ec4984b168db0e12a539cc0c83cd' ? 'ETH' : token0Address}/${'ETH'}`
    )
  } else {
    return (
      `https://carbonpaws.io/exchange/` +
      (remove ? `remove` : `add`) +
      `/${token0Address === '0x6b3bd0478df0ec4984b168db0e12a539cc0c83cd' ? 'ETH' : token0Address}/${
        token1Address === '0x6b3bd0478df0ec4984b168db0e12a539cc0c83cd' ? 'ETH' : token1Address
      }`
    )
  }
}
...
export function getSwapLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `https://carbonpaws.io/exchange/swap?inputCurrency=${token0Address}`
  } else {
    return `https://carbonpaws.io/exchange/swap?inputCurrency=${
      token0Address === '0x6b3bd0478df0ec4984b168db0e12a539cc0c83cd' ? 'ETH' : token0Address
    }&outputCurrency=${token1Address === '0x6b3bd0478df0ec4984b168db0e12a539cc0c83cd' ? 'ETH' : token1Address}`
  }
}

export function getMiningPoolLink(token0Address) {
  return `https://carbonpaws.io/#/uni/ETH/${token0Address}`
}

export function getCarbonPawsAppLink(linkVariable) {
  let baseCarbonPawsUrl = 'https://carbonpaws.io/#/uni'
  if (!linkVariable) {
    return baseCarbonPawsUrl
  }

  return `${baseCarbonPawsUrl}/ETH/${linkVariable}`
}
