...
export default function Warning({ type, show, setShow, address }) {
  ...
  const textContent = below800 ? (
    <div>
      ...
      <Text fontWeight={500} lineHeight={'145.23%'} mt={'10px'}>
        Similar to EW Explorer, this site automatically tracks analytics for all ERC20 tokens independent of token
    ...
    <Text fontWeight={500} lineHeight={'145.23%'} mt={'10px'}>
      Anyone can create and name any ERC20 token on Ethereum, including creating fake versions of existing tokens and
      tokens that claim to represent projects that do not have a token. Similar to EW Explorer, this site automatically
          ...
          <div>
            <Hover style={{ marginTop: '10px' }}>
              <Link
                ...
                target="_blank"
              >
                View {type === 'token' ? 'token' : 'pair'} contract on EW Explorer
              </Link>
            </Hover>
