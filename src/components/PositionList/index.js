...
const DashGrid = styled.div`
  ...
  grid-template-areas: 'number name CarbonPaws return';
  ...
  @media screen and (max-width: 740px) {
    ...
    grid-template-areas: 'name CarbonPaws return';
  }

  @media screen and (max-width: 500px) {
    grid-template-columns: 2.5fr 1fr;
    grid-template-areas: 'name CarbonPaws';
  }
`
...
const SORT_FIELD = {
  VALUE: 'VALUE',
  CarbonPaws_RETURN: 'CarbonPaws_RETURN',
}
  ...
  const positionsSorted =
    ...
    positions
      ...
      .sort((p0, p1) => {
        ...
        if (sortedColumn === SORT_FIELD.CarbonPaws_RETURN) {
          return p0?.CarbonPaws?.return > p1?.CarbonPaws?.return ? (sortDirection ? -1 : 1) : sortDirection ? 1 : -1
        }
        ...
        {!below500 && (
          <Flex alignItems="center" justifyContent="flexEnd">
            <ClickableText
              area="return"
              onClick={() => {
                setSortedColumn(SORT_FIELD.CarbonPaws_RETURN)
                setSortDirection(sortedColumn !== SORT_FIELD.CarbonPaws_RETURN ? true : !sortDirection)
              }}
            >
              {below740 ? 'Fees' : 'Total Fees Earned'}{' '}
              {sortedColumn === SORT_FIELD.CarbonPaws_RETURN ? (!sortDirection ? '↑' : '↓') : ''}
            </ClickableText>
          </Flex>
        )}
      </DashGrid>
