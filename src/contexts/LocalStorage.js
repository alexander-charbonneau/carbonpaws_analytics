...
function init() {
  ...
  try {
    const parsed = JSON.parse(window.localStorage.getItem(CarbonPaws))
...
export function Updater() {
  ...
  useEffect(() => {
    window.localStorage.setItem(CarbonPaws, JSON.stringify({ ...state, [LAST_SAVED]: Math.floor(Date.now() / 1000) }))
  })
