...
const UPDATE_ALL_PAIRS_IN_CarbonPaws = 'UPDAUPDATE_ALL_PAIRS_IN_CarbonPawsTE_TOP_PAIRS'
const UPDATE_ALL_TOKENS_IN_CarbonPaws = 'UPDATE_ALL_TOKENS_IN_CarbonPaws'
...
function reducer(state, { type, payload }) {
  switch (type) {
    ...
    case UPDATE_ALL_PAIRS_IN_CarbonPaws: {
    ...
    case UPDATE_ALL_TOKENS_IN_CarbonPaws: {
...
export default function Provider({ children }) {
  ...
  const updateAllPairsInCarbonPaws = useCallback((allPairs) => {
    dispatch({
      type: UPDATE_ALL_PAIRS_IN_CarbonPaws,
      payload: {
        allPairs,
      },
    })
  ...
  const updateAllTokensInCarbonPaws = useCallback((allTokens) => {
    dispatch({
      type: UPDATE_ALL_TOKENS_IN_CarbonPaws,
      payload: {
        allTokens,
      },
    })
      ...
      value={useMemo(
        () => [
          state,
          {
            ...
            updateEthPrice,
            updateTopLps,
            updateAllPairsInCarbonPaws,
            updateAllTokensInCarbonPaws,
          },
        ],
        [
          state,
...
/**
 * Loop through every pair on CarbonPaws, used for search
 */
async function getAllPairsOnCarbonPaws() {
  try {
    let allFound = false
    let pairs = []
    let skipCount = 0
...
/**
 * Loop through every token on CarbonPaws, used for search
 */
async function getAllTokensOnCarbonPaws() {
  try {
    let allFound = false
    let skipCount = 0
    let tokens = []
...
export function useGlobalData() {
  const [state, { update, updateAllPairsInCarbonPaws, updateAllTokensInCarbonPaws }] = useGlobalDataContext()
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      let allPairs = await getAllPairsOnCarbonPaws()
      updateAllPairsInCarbonPaws(allPairs)

      let allTokens = await getAllTokensOnCarbonPaws()
      updateAllTokensInCarbonPaws(allTokens)
    }
    if (!data && ethPrice && oldEthPrice) {
      fetchData()
    }
  }, [ethPrice, oldEthPrice, update, data, updateAllPairsInCarbonPaws, updateAllTokensInCarbonPaws])
...
export function useAllPairsInCarbonPaws() {
...
export function useAllTokensInCarbonPaws() {
  const [state] = useGlobalDataContext()
