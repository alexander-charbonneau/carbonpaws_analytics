...
function AccountPage({ account }) {
  ...
  return (
    <PageWrapper>
      <ThemedBackground backgroundColor={transparentize(0.6, '#f2cd00')} />
      <ContentWrapper>
        <RowBetween>
          <TYPE.body>
            <BasicLink to="/accounts">{'Accounts '}</BasicLink>→{' '}
            <Link lineHeight={'145.23%'} href={'https://explorer.energyweb.org/address/' + account} target="_blank">
              {' '}
              {account?.slice(0, 42)}{' '}
            </Link>
          </TYPE.body>
        ...
        <Header>
          <RowBetween>
            <span>
              ...
              <Link lineHeight={'145.23%'} href={'https://explorer.energyweb.org/address/' + account} target="_blank">
                <TYPE.main fontSize={14}>View on EW Explorer</TYPE.main>
              </Link>
            </span>
            <AccountWrapper>
              <StyledIcon>
            ...
            <DropdownWrapper>
              ...
              {showDropdown && (
                <Flyout>
                  <AutoColumn gap="0px">
                    {positions?.map((p, i) => {
                      if (p.pair.token1.symbol === 'WEWT') {
                        p.pair.token1.symbol = 'EWT'
                      }
                      if (p.pair.token0.symbol === 'WEWT') {
                        p.pair.token0.symbol = 'EWT'
                      }
                      return (
